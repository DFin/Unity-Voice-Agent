...
- [x] Implement audio playback pipeline (PCM decoding, streaming audio queue with resampling + large buffer) with optional spatialization flag for VR targets.
...
- [x] Route server `input_audio_buffer.speech_started` events to clear local playback and expose manual cancellation helpers.
